...
## OVERVIEW
Arch Linux + Hyprland dotfile management and system provisioning repository. Uses GNU Stow for configuration symlinking and a custom shell-based orchestration for system setup.
...
- **Numerical Prefix**: Use `00-` for scripts that must run early (e.g., stow setup).
...
## NOTES
- Tightly coupled with the parent "Omarchy" system (requires `omarchy-webapp-*` commands).
